...
export default {
  async fetch(request, env, ctx) {
    ...
    if (text.split(/\s+/)[0] === "/coins") {
      ...
      // У человека ровно один device, выведенный из tg_id. Раньше здесь искали
      // «последнее привязанное устройство», а их набегало по одному на каждое
      // открытие — монеты уходили на личность, которой телефон уже не пользуется.
