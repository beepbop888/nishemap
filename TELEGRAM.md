...
## Модерация: жалобы приходят кнопками в Telegram

Человек жмёт «неверно» в шторке → сайт пишет в `reports` и дёргает воркер →
воркер считает жалобы и присылает владельцу карточку с тремя кнопками:

| Кнопка | Что пишет в `overrides` | Что видно на карте |
|---|---|---|
| 🚫 Скрыть | `hidden = true` | позиции нет ни в списке, ни в шторке |
| ⚪ Серая монета | `disputed = true` | монета серая со знаком «?», цена обесцвечена, кнопка «Знаю настоящую цену» |
| ✅ Всё верно | обе `false` | как было |

Три жалобы делают монету серой сами, но **скрыть может только человек** —
иначе три клика тролля стирают место с карты.

Чтобы чат не превращался в свалку, воркер шумит не на каждую жалобу, а на
1-й, 3-й, 10-й, 30-й и 100-й. Кнопки принимаются только от `OWNER_CHAT_ID`.

Серая монета — не наказание, а вопрос: место остаётся на карте и просит
настоящую цену. Кнопка «Знаю настоящую цену» открывает форму с уже вписанным
заведением и блюдом.

Таблица решений — `supabase/13_overrides.sql`, одна на сидовые точки
(`i101`) и на присланные людьми (`ui-<uuid>`).
...
## Чтобы карта не уснула

Supabase на бесплатном тарифе усыпляет проект примерно после недели без
обращений. Уснувший проект — это разом: цены, монеты, модерация, ДрищMap.

Сторожей теперь два, и оба проверены:

| Кто | Как часто | Чем бьёт |
|---|---|---|
| `.github/workflows/keepalive.yml` | каждые 6 часов | публичным ключом, БЕЗ секретов |
| воркер, cron `17 */6 * * *` | каждые 6 часов | служебным ключом |

Пульс в Actions нарочно не требует ни одного секрета и **падает**, если база
ответила не 200. Молчаливая зелёная галочка здесь опаснее красного креста —
именно так месяц «работала» рассылка, у которой не было секретов.

Проверено 2026-09-14: `Supabase ответил: 200`.

⚠️ Пульс воркера при проверке не сработал ни разу — расписание Cloudflare
принял, но события не пришло. Поэтому главный сторож — тот, что в Actions.
Разобраться с воркером стоит, но рассчитывать пока не на него.

⚠️ GitHub отключает расписания в репозитории, где 60 дней не было коммитов.
Раз в пару месяцев достаточно любого коммита.

### Рассылка не работала ни разу

В секретах репозитория не было ни одного из трёх значений, и `digest.mjs`
каждые 6 часов честно писал «секреты не заведены» и выходил с кодом 0 — в
Actions это выглядело как успех. Пятничный топ-5 не уходил никогда.

Заведены: `BOT_TOKEN`, `SUPABASE_URL`. Остался `SUPABASE_SERVICE_KEY` —
его класть человеку, из своего терминала:

```bash
gh secret set SUPABASE_SERVICE_KEY   # вставить ключ service_role
```
