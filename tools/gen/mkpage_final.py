...
def build(fixed_notes, title, outfile):
    ...
    for cid,name,price,theme,sex in CHARS:
        ...
        cards+=(f'<figure class="sh t{TIER(price)}{" is-new" if fx else ""}">'
            f'<div class="frame"><div class="inner bg-{theme}">'
...
.inner img{{position:absolute;left:50%;bottom:0;transform:translateX(-50%);
 height:88%;max-width:94%;object-fit:contain;object-position:bottom center;z-index:1;
 filter:drop-shadow(0 2px 5px rgba(0,0,0,.45))}}
/* имя: крупнее и жирнее, лента выше */
.rib{{position:absolute;left:-3px;right:-3px;bottom:13%;padding:5px 3px;text-align:center;
 font:700 11.5px/1.15 Oswald,Impact,sans-serif;letter-spacing:.04em;color:#fff;
